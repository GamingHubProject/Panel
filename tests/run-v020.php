...
$check(($plugin['version'] ?? null) === '0.2.1', 'plugin version is 0.2.1');
$check(($manifest['version'] ?? null) === '0.2.1', 'extension manifest version is 0.2.1');
$check(($plugin['dependencies']['gaming-hub-core'] ?? null) === '>=0.6.0 <0.8.0', 'Gaming Hub Core compatibility includes reviewed 0.7.x line');
...
$check(str_contains($serviceProvider, "ProviderConfigurationField('panel_connection_id'"), 'provider type declares connection mapping field');
$check(str_contains($serviceProvider, "ProviderConfigurationField('panel_server_identifier'"), 'provider type declares stored identifier field');
