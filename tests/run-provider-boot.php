...
namespace {
    ...
    file_put_contents($coreRoot.'/plugin.json', json_encode(['id' => 'gaming-hub-core', 'version' => '0.7.0'], JSON_THROW_ON_ERROR));
    ...
    $check(array_keys($types->types) === [], 'Pelican and Pterodactyl types are not registered (P0 stub)');
    $check(count($readers->readers['pelican'] ?? []) === 0, 'Pelican readers are not registered (P0 stub)');
    $check(count($readers->readers['pterodactyl'] ?? []) === 0, 'Pterodactyl readers are not registered (P0 stub)');
    ...
    $check($status['pelican_provider_type_registered'] === false, 'boot diagnostics Pelican false (P0 stub)');
    $check($status['pterodactyl_provider_type_registered'] === false, 'boot diagnostics Pterodactyl false (P0 stub)');
    $check(View::$namespaces === [], 'Core view namespaces are not overridden (P0 removed the overrides)');
