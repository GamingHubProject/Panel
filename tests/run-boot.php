...
$check(! $result->compatible && $result->code === 'core_version_incompatible', 'incompatible Core rejected clearly');
$check(str_contains($result->reason, '>=0.6.0 <0.7.0'), 'incompatible result states supported range');
...
$status->recordCompatibility($compatibility($core062)->inspect());
$status->markRoutesRegistered();
$status->markProviderRegistered('pelican');
$status->markProviderRegistered('pterodactyl');
$snapshot = $status->snapshot();
$check($snapshot['panel_version'] === '0.2.0', 'diagnostics reports Panel version');
$check($snapshot['core_version'] === '0.6.2', 'diagnostics reports Core version');
