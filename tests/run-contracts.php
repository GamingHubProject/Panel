...
$check(($plugin['version'] ?? null) === '0.2.1', 'release version 0.2.1');
$check(($plugin['dependencies']['gaming-hub-core'] ?? null) === '>=0.6.0 <0.8.0', 'Azuriom Core dependency supports reviewed Core lines');
$check(! array_key_exists('gaming-hub-core', $manifest['requires'] ?? []), 'legacy top-level Core manifest requirement removed'
);
$check(($manifest['requires']['extensions']['gaming-hub-core'] ?? null) === '>=0.6.0 <0.8.0', 'Manager extension Core dependency declared');
...
$check(($registry['extensions'][0]['latest_version'] ?? null) === '0.2.1', 'registry version updated to 0.2.1');
...
$check(str_contains($compatibility, "VERSION_CONSTRAINT = '>=0.6.0 <0.8.0'"), 'Core compatibility range bounded');
