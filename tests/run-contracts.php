...
$check(($plugin['version'] ?? null) === '0.1.010', 'release version 0.1.010');
$check(($plugin['dependencies']['gaming-hub-core'] ?? null) === '*', 'Azuriom Core dependency is required presence-only');
$check(! array_key_exists('gaming-hub-core', $manifest['requires'] ?? []), 'legacy top-level Core manifest requirement removed'
);
$check(($manifest['requires']['extensions']['gaming-hub-core'] ?? null) === '*', 'Manager extension Core dependency is required presence-only');
$check(($manifest['type'] ?? null) === 'integration', 'integration type');
$check(($plugin['url'] ?? null) === 'https://github.com/GamingHubProject/Panel', 'plugin repository metadata current');
$check(($manifest['repository'] ?? null) === 'https://github.com/GamingHubProject/Panel', 'manifest repository metadata current');
$check(! str_contains(file_get_contents($root.'/plugin.json').file_get_contents($root.'/gaming-hub-extension.json').file_get_contents($root.'/examples/extensions.json'), 'RosesOfDorns'), 'legacy RosesOfDorns metadata removed');
$check(($manifest['package']['plugin_directory'] ?? null) === 'gaming-hub-panel', 'single package root contract');
$check(($manifest['provides']['providers'] ?? []) === [], 'provider declarations are empty pending Connector extraction (P0)');
...
$check(($registry['extensions'][0]['latest_version'] ?? null) === '0.1.010', 'example registry version updated to 0.1.010');
...
$check(str_contains($compatibility, "VERSION_CONSTRAINT = '*'"), 'Core compatibility dependency is presence-only');
...
$check(! str_contains($serviceProvider, "['server-status', 'metrics']"), 'no capabilities are advertised while registration is stubbed (P0)');
...
$output = [];
exec('php '.escapeshellarg($root.'/tests/run-connector-sdk.php').' 2>&1', $output, $code);
foreach ($output as $line) {
    echo 'CONNECTOR-SDK '.$line.PHP_EOL;
}
$check($code === 0, 'Connector SDK proof harness');
