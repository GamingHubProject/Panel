...
final class CoreCompatibilityTest extends TestCase
{
    ...
    public function testPresenceOnlyDependencyAcceptsAnyReportedCoreVersion(): void
    {
        foreach (['0.7.170', '0.8.100', '0.8.110', '0.9.0', '1.0.0'] as $version) {
            ...
            self::assertSame('compatible', $result->code);
        }
    }

    public function testMissingCoreStillFailsBecauseDependencyIsMandatory(): void
    {
    ...
    {
        $result = $this->compatibility($this->metadata('0.8.110'), interfaces: false)->inspect();
    ...
    public function testRequiredClassesRemainRuntimeChecked(): void
    {
        $result = $this->compatibility($this->metadata('0.8.110'), classes: false)->inspect();

        self::assertFalse($result->compatible);
        self::assertSame('core_contract_missing', $result->code);
        self::assertStringContainsString('type', $result->reason);
    }
