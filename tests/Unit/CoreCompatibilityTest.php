...
final class CoreCompatibilityTest extends TestCase
{
    ...
    public function testCoreZeroSixTwoIsAccepted(): void
    {
        $result = $this->compatibility($this->metadata('0.6.2'))->inspect();

        self::assertTrue($result->compatible);
        self::assertSame('0.6.2', $result->coreVersion);
    }
    ...
    public function testMissingAndIncompatibleCoreHaveSafeReasons(): void
    {
        ...
        $incompatible = $this->compatibility($this->metadata('0.7.0'))->inspect();
        self::assertFalse($incompatible->compatible);
        self::assertSame('core_version_incompatible', $incompatible->code);
        self::assertStringContainsString('>=0.6.0 <0.7.0', $incompatible->reason);
    }
