...
INSTALLER_VERSION="2.4-manager-upgrade"
...
PY_CURRENT
    ...
    if ! curl -fsSL --retry 3 --retry-delay 2 \
        ...
        "$MANAGER_API" -o "$upgrade_tmp/release.json"; then
        rm -rf "$upgrade_tmp"
...
PY_RELEASE
    ...
    local latest_tag manager_url asset_digest
    IFS=$'\t' read -r latest_tag manager_url asset_digest <<< "$release_info"
    [[ -n "$latest_tag" && -n "$manager_url" ]] || {
        rm -rf "$upgrade_tmp"
        fail "The latest Manager release metadata is incomplete."
    }

    if ! curl -fL --retry 3 --retry-delay 2 "$manager_url" -o "$upgrade_tmp/manager.zip"; then
        rm -rf "$upgrade_tmp"
        fail "Could not download ${latest_tag}."
    fi
    ...
    if [[ "$asset_digest" == sha256:* ]]; then
        ...
        if [[ "$actual_sha" != "$expected_sha" ]]; then
            rm -rf "$upgrade_tmp"
            fail "Manager release SHA-256 verification failed."
        fi
...
with open(sys.argv[1], encoding="utf-8") as f:
    enabled = json.load(f)
raise SystemExit(0 if "gaming-hub-manager" in enabled else 1)
PY_ENABLED
        then
            enabled_before="yes"
        fi
